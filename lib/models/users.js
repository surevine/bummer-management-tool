...
var deleteUser = function(admin, jid, callback) {
    debug('Deleting user', jid)
    if ('admin' === jid.user) {
        return callback({ message: 'Sorry, this is a restricted user' })   
    }
    storage.getServerCredentials(admin, function(error, credentials) {
        ...
        debug('Connect to XMPP server using', credentials)
        if (!credentials) {
            return callback({
                message: 'You are not an admin and therefore can not obtain user details',
                type: 'authorisation'
        ...
        xmpp.deleteUser(credentials, jid, function(error, success) {
            if (error) {
                error.type = 'xmpp'
                if ('XMPP authentication failure' === error.message) {
                    error.message = 'Could not access your XMPP account, please contact support'
...
var resetPassword = function(admin, options, callback) {
    debug('Resetting user password', options.jid)
    if ('admin' === options.jid.user) {
        return callback({ message: 'Sorry, this is a restricted user', type: 'restricted' })   
    }
    storage.getServerCredentials(admin, function(error, credentials) {
        if (error) {
            error.message = 'We experienced a server problem, apologies!'
        ...
        xmpp.resetPassword(credentials, options, function(error, success) {
            if (error) {
                error.type = 'xmpp'
                if ('XMPP authentication failure' === error.message) {
                    error.message = 'Could not access your XMPP account, please contact support'
...
var createUser = function(admin, options, callback) {
    ...
    if (!options.jid.user.match(/^[a-z0-9\_\-\.]{3,70}$/i)) {
    ...
    storage.getServerCredentials(admin, function(error, credentials) {
        ...
        if (!credentials) {
            return callback({
                message: 'You are not an admin and therefore can not add a user',
                type: 'authorisation'
            })
        }
